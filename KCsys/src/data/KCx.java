...
public class KCx {
	...
	public JTable mt(JFrame mf, String[][] arr, String u) {
		...
		JTable maintable = new JTable() {
			...
			public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
				...
				if (columnIndex == 4) {
					int scf = JOptionPane.showConfirmDialog(mf, "是否修改", "修改提醒", JOptionPane.YES_NO_OPTION);
					if (scf == JOptionPane.YES_OPTION) {
						String st = (String) aValue;
						new SQLFilter(mf, st, u);
					...
					if (r >= 0) {
						...
						if (sr > 0) {
							...
							if (isChange == JOptionPane.YES_OPTION) {
								...
								wx.changeType(chooseType, maintable.getValueAt(r, 0).toString(), s, typeList[sr - 1][0],
										typeList[sr - 1][1]);
		...
		String[] cn = { "编号", "种类", "供应商编号", "供应商名称", "名称", "进货价", "分销价", "经销价", "单价", "数量", "单位", "修改日期", "警告数量",
				"库存位置" };
	...
	public KCx(String user) {
		...
		delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String na = mtt.getValueAt(r, 4).toString();
				int scf = JOptionPane.showConfirmDialog(mainf, "是否删除\n" + na, "删除提醒", JOptionPane.YES_NO_OPTION);
				if (scf == JOptionPane.YES_OPTION) {
					wx.deleteProduct(mainf, sbh);
					DefaultTableModel dm = (DefaultTableModel) mtt.getModel();
					dm.removeRow(r);
					Double hjjhj = 0.0;
					int cr = mtt.getRowCount();
					for (int i = 0; i < cr; i++) {
						String sjhj = mtt.getValueAt(i, 5).toString();
						String ssl = mtt.getValueAt(i, 9).toString();
						Double jhj = Double.parseDouble(sjhj);
		...
		cxt.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				// TODO Auto-generated method stub
				if (e.getKeyCode() == '\n') {
					new SQLFilter(mainf, cxt.getText(), user);
					int zl = 0;
					if (cxzl.getSelectedIndex() == 0) {
						mtt = mt(mainf, wx.KCdata(cxt.getText(), "", ""), user);
					} else {
						zl = cxzl.getSelectedIndex();
						mtt = mt(mainf, wx.KCdata(cxt.getText(), jczl[zl - 1][0], ""), user);
					}
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyChar() == '\n') {
					try {
						...
						int tr = mtt.getSelectedRow();
						// Double
						// jhj=Double.parseDouble(mtt.getValueAt(tr,5).toString());
